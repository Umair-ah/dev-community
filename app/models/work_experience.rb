class WorkExperience < ApplicationRecord
  ...
  validate :end_date_greator_than_start_date, if: :currently_not_working_here?
  ...
  def presence_of_end_date
    if end_date.nil? && !currently_working_here
      errors.add(:end_date, "must be present if you are not currently working in the company")
    end
  end
  ...
  def end_date_greator_than_start_date
    return if end_date.nil?

    if end_date < start_date
      errors.add(:end_date, "must be greator than start date")
    end
  end
